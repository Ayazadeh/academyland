	...
	<template v-if="type === SkeletonTypes.AVATAR_LIST">
		...
		>
			<div
				...
				class="relative overflow-hidden animate-pulse delay-500 flex items-center space-x-4"
			>
				<div
					class="skeleton-box absolute inset-0 transform -translate-x-full"
				></div>
				<div
...
const props = withDefaults(defineProps<{
    containerClass?: Record<string, any> | string[];
    type?: SkeletonTypes;
    border?: boolean;
    shadow?: boolean;
    rounded?: boolean;
}>(), {
    type: SkeletonTypes.AVATAR_LIST,
    border: true,
    shadow: true
})
